...
pub(super) const REQUEST_ID_INVARIANT_SENTINEL: &str = "request-id-invariant-failure";
...
impl RequestId {
    ...
    pub(super) fn invariant_failure() -> Self {
        Self(Arc::from(REQUEST_ID_INVARIANT_SENTINEL))
    }
}

pub(super) fn request_id(request: &Request) -> Option<RequestId> {
    request.extensions().get::<RequestId>().cloned()
}

pub(super) fn valid_request_id(value: &str) -> bool {
    value != REQUEST_ID_EXHAUSTED_SENTINEL
        && value != REQUEST_ID_INVARIANT_SENTINEL
        && !value.is_empty()
