...
- [`../README.md`](../README.md) — project boundary, architecture, quick start, hosted API reference, Python client, MCP entry point, and documentation entry points.
...
- [`MCP_SERVER.md`](MCP_SERVER.md) — current MCP architecture, installation, stdio/Streamable HTTP transports, complete configuration, security boundary, tool capability policy, and unknown-outcome recovery.
...
- [`../python/README.md`](../python/README.md) — Python client installation, direct GitHub installation, API usage, and the `vnc-remote-control-demo` / `vnc-remote-control-mcp` entry points.
- [`../deploy/README.md`](../deploy/README.md) — current Docker Compose topology, secrets, persistence, debug VNC, custom desktop overrides, and MCP co-location boundary.
...
- [`../SECURITY.md`](../SECURITY.md) — current security boundaries, secret lifecycle guarantees, MCP Python-memory limitation, and explicit residual risks.
...
The MCP adapter has a separate living guide at [`MCP_SERVER.md`](MCP_SERVER.md). MCP is not part of the controller's OpenAPI surface: it is a Python adapter that calls the authenticated controller API through the typed client.
...
- correctness-review, shutdown-hardening, post-correctness-hardening, final-polish, cleanup, and MCP SPEC/TODO/EVIDENCE sets — the design and execution record for those passes.
...
2. update `openapi.json` and WebSocket documentation when their public controller contracts change;
3. update `MCP_SERVER.md` when MCP configuration, transports, tool capabilities, SDK pin, security semantics, or mutation-outcome behavior changes;
4. update the Python client/demo/MCP documentation when its install or callable surface changes;
5. update deployment, desktop-component, and security documentation when topology, secrets, dependencies, images, or trust boundaries change;
6. update current engineering-support documentation when its commands, paths, or executable measurement/test contracts change;
7. add or strengthen documentation contract tests when a stale statement could otherwise recur;
8. leave point-in-time historical artifacts intact unless the change is specifically about correcting that historical record.
