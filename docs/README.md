...
These documents are expected to track the behavior and supported configuration of the current repository:
...
4. update deployment/security documentation when topology, secrets, dependencies, or trust boundaries change;
5. add or strengthen documentation contract tests when a stale statement could otherwise recur;
6. leave point-in-time historical artifacts intact unless the change is specifically about correcting that historical record.
