"""Contract tests asserting the WorkerHandle input E2E driver and harness stay correct."""
...
class WorkerInputE2EContractTests(unittest.TestCase):
    """Asserts the WorkerHandle input E2E driver, harness, and CI wiring stay correct."""

    def test_driver_uses_production_worker_and_all_required_inputs(self) -> None:
        """The Rust driver exercises the production DesktopWorker with all required input
        commands."""
    ...
    def test_harness_verifies_deterministic_desktop_state(self) -> None:
        """The shell harness verifies the deterministic desktop-app state after the driver runs."""
    ...
    def test_harness_captures_redacted_failure_diagnostics(self) -> None:
        """The shell harness captures failure diagnostics with secrets redacted."""
        text = HARNESS.read_text(encoding="utf-8")
    ...
    def test_authoritative_ci_runs_the_worker_e2e(self) -> None:
        """The authoritative CI workflow runs the WorkerHandle input E2E harness."""
        text = CI.read_text(encoding="utf-8")
        self.assertIn("tests/worker-e2e/run.sh", text)
        self.assertIn("Run WorkerHandle TigerVNC input E2E test", text)

    def test_authoritative_ci_uploads_failure_only_diagnostics(self) -> None:
        """CI uploads WorkerHandle E2E diagnostics only when the harness step fails."""
        text = CI.read_text(encoding="utf-8")
    ...
    def test_authoritative_ci_exercises_and_redacts_failure_diagnostics(self) -> None:
        """CI forces a failure to prove diagnostics capture works and secrets are redacted."""
        text = CI.read_text(encoding="utf-8")
