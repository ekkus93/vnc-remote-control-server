...
def _assert_body_and_coordinate_bounds(harness: Harness) -> None:
    ...
    require(
        invalid_coordinate.status == 422
        and error_code(invalid_coordinate) == "invalid_coordinate",
        "coordinate limit was not explicit",
    )
    scroll = post_json(harness, "/v1/pointer/scroll", {"x": 1, "y": 1, "delta_y": 101})
    require(
        scroll.status == 422 and error_code(scroll) == "scroll_too_large",
        "scroll limit was not explicit",
    )
...
def _assert_queue_saturation(harness: Harness) -> None:
...
def _assert_concurrent_screenshot_overload(harness: Harness) -> None:
    ...
    require(
        all(result.status in {200, 503} for result in screenshots),
        "concurrent screenshots returned unexpected status",
    )
    require(
        any(result.status == 200 for result in screenshots), "no concurrent screenshot succeeded"
    )
...
def assert_abuse_and_concurrency(harness: Harness) -> None:
    ...
    _assert_body_and_coordinate_bounds(harness)
    _assert_queue_saturation(harness)
    _assert_reconnect_rate_limit(harness)
    _assert_concurrent_screenshot_overload(harness)


def assert_reconnect_and_resource_bounds(harness: Harness) -> None:
    """Verify desktop restart, framebuffer invalidation, reconnect, and resource bounds."""
    harness.log(
        "verifying desktop restart detection, framebuffer invalidation, reconnect, "
        "and resource bounds"
    )
    ...
    for cycle in range(3):
        ...
        require(
            status.get("state") in {"degraded", "disconnected", "reconnecting", "connecting"},
            f"unexpected disconnect state: {status}",
        )
        ...
        require(
            display.status == 503 and error_code(display) == "framebuffer_unavailable",
            "old display remained available",
        )
        require(
            screenshot.status == 503 and error_code(screenshot) == "framebuffer_unavailable",
        ...
        require(
            bool(new_etag) and new_etag != previous_etag,
            "ETag did not change after framebuffer invalidation",
        )
        assert new_etag is not None  # narrows for mypy; require() already enforced this
    ...
    require(
        threads <= baseline_threads + 4,
        f"worker thread count grew materially: {baseline_threads} -> {threads}",
    )
    rss_allowance = max(32768, baseline_rss // 2)
    require(
        rss <= baseline_rss + rss_allowance,
        f"controller RSS grew materially: {baseline_rss} -> {rss} KiB",
    )
