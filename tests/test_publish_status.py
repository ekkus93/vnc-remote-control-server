...
def event(*, status: str = "completed", conclusion: str | None = "success", run_id: int = 101):
    return {
        "workflow_run": {
            "name": "CI",
            "workflow_id": 777,
            "id": run_id,
            "run_attempt": 1,
            "html_url": f"https://github.com/ekkus93/vnc-remote-control-server/actions/runs/{run_id}",
            "event": "push",
            "status": status,
...
def step(number: int, name: str, conclusion: str = "success"):
    return {
        "number": number,
        "name": name,
        "status": "completed",
...
def job(job_id: int, name: str, conclusion: str | None = "success", steps=None):
    return {
        "id": job_id,
        "name": name,
...
def artifact(artifact_id: int = 301):
    return {
        "id": artifact_id,
        "name": "ci-evidence",
        "size_in_bytes": 1234,
...
def payload_for(*, event_value=None, jobs=None, artifacts=None):
...
class PublishStatusTests(unittest.TestCase):
    def test_completed_success_contains_jobs_steps_and_artifacts(self):
    ...
    def test_requested_run_does_not_fabricate_successful_jobs(self):
    ...
    def test_failed_step_is_reported_with_exact_job_id(self):
        failing_job = job(
            202,
            "quality `special`",
            "failure",
    ...
    def test_successful_steps_are_compacted_without_invalid_json(self):
    ...
    def test_abnormal_steps_are_not_compacted_away(self):
    ...
    def test_same_run_cannot_regress_from_completed_to_in_progress(self):
    ...
    def test_newer_run_supersedes_old_run(self):
        existing = payload_for(jobs=[job(201, "quality")], artifacts=[artifact()])
        incoming = payload_for(event_value=event(run_id=102), jobs=[job(203, "quality")], artifacts=[])
        publish, reason = should_publish(existing, incoming)
        self.assertTrue(publish)
        self.assertEqual(reason, "newer_run")

    def test_existing_issue_identity_mismatch_fails_closed(self):
    ...
    def test_malformed_existing_json_is_rejected(self):
        body = f"{MARKER}\n```json\n{{not-json}}\n```\n"
        with self.assertRaises(PayloadError):
            extract_existing_payload(body)

    def test_payload_round_trips_as_machine_json(self):
        payload = payload_for(jobs=[job(201, "quality")], artifacts=[artifact()])
